...
- Each session runs `tau --mode json --no-extensions --no-approve -e extensions/superpowers-subagent "<case prompt>"` as a subprocess with the ambient harness configuration; the script resolves the `-e` extension path against the repository root regardless of the caller's working directory. `--no-extensions` keeps the explicitly named repository extension the only loaded copy when an installed same-name extension exists (Tau's loader dedupes by name and the user dir precedes `-e` extras), so the evaluation exercises the repository's implemented surface. It parses the stdout JSON lines for assistant tool-call events, and records per case: the tool name of every task-surface call, the complete JSON argument object, every fail-closed teach-back content, and the number of launched children (envelopes with a completed or error state).
- The four cases and their prompts:
  ...
  3. `unknown-resume`: "Use the task_resume tool with task_id set to the exact text nonexistent and the prompt 'Report status.' Wait for the result and relay it verbatim." Measure: the rejected call records the fail-closed contract — its result content is the teach-back that preserves `nonexistent` and directs the caller to `task` — so the rejected call itself launches zero children; the session records any follow-up call the model makes as an observation.
  4. `camelcase`: "Use the task tool to dispatch one general-purpose subagent. Pass timeoutSeconds with the value 120 as a call argument, exactly that spelling. Relay the result." Measure: the rejected call records the fail-closed contract and the teach-back names `timeoutSeconds` and shows `timeout_seconds`, so the rejected call itself launches zero children; the session records whether the model emitted the camelCase field and what it did after the teach-back as the residual-rate observation.
- The script exits 0 when the hard measures hold: in case 3 and case 4, every rejected call records the fail-closed contract (its own result carries no envelope, an empty `results` array, and the teach-back), so the rejected call itself launches zero children, and each of case 3 and case 4 records at least one rejected call, so the proof is not vacuous. The proposal's key measures are zero accidental launches after a failed resume and a recorded residual camelCase rate; a follow-up call the model makes after a teach-back is a legitimate new dispatch, so the session-level child count is recorded, not gated. Cases 1 and 2 are recorded without hard thresholds. A case 2 failure to resume is a recorded observation, not a gate. Exit code 1 means a hard measure failed or a session failed to run and fails this task's gate. Exit code 3 means the ambient harness has no configured provider: the report records the blocked state and the gate accepts exit 3 with the blocked report, because the evaluation's value depends on the provider the harness configures and the proposal requires no specific provider.
