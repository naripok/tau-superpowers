...
- The roster is the model-visible agent list: bundled, user, and project agents, discovered at session start from the parent session cwd.
...
Material baseline discrepancies and their resolution: the living spec records requirements this change dissolves, reverses, or rewrites. The finishing-stage living-spec sync of the finishing-a-development-branch skill records each, on the basis of this approved proposal:

- The placeholder-coercion rule (a case-insensitive `default`, `inherit`, or `auto` value on `provider`, `model`, or `reasoningEffort` coerces to omitted with a repair note) dissolves: the coerced parameters no longer exist. The rejection-versus-coercion discrepancy recorded during the previous interface change was already resolved by the spec sync at commit `06ff6b6`.
- The task_id-resume requirement's "The tool SHALL NOT record a per-session agent owner" and its "Resume under a different agent" scenario reverse into the session-agent mapping of Required Outcome 6, and the different-agent resume capability is removed.
- The "Explicit project-agent approval" requirement and the port-differences row "Project-agent prompt in headless use" reverse: the approval flow and `confirm_project_agents` are removed, and project-layer definitions dispatch without per-call approval.
- The roster and teach-back exclusions ("Project agents SHALL stay out of every roster and every teach-back, including by name") reverse: the all-layer roster and `task` teach-backs name project agents.
- The "Unknown task_id fallback" requirement (a fresh child with a repair note) reverses into the fail-closed resume of Required Outcome 7.
- The details mandate that carries the agent scope and the project agents directory dissolves: the `agentScope` and `projectAgentsDir` details fields are removed.
- The "Resume working directory" repair-note rule dissolves with the repair-note mechanism.
- The "task_id SHALL require subagent_type" rule reverses: `task_resume` requires no `subagent_type` and rejects it as an unknown field.
- The "Agent definition discovery" requirement's scope-selection sentences ("`agentScope: user` SHALL be the default and include bundled plus user definitions", the `project` and `both` sentences, and the "User scope" scenario) dissolve under the fixed all-layer discovery of Required Outcome 5, beside the key rename and stale-key exception in the same requirement.
- The "Provider, model, and reasoning-effort overrides" requirement's call-level legs (the optional-literal-override SHALL and the resolution chain's call-level first leg) dissolve under Required Outcome 4, beside the coercion rule and the recovery-note rewrite.
- The "Task result envelope" requirement's "Repair notes SHALL appear as `Note:` lines before the envelope" and the "Repair note placement" scenario dissolve under the repair-note removal of Required Outcome 8.
- The "Pinned child sessions" requirement's fallback sentence and the "Recovery removes the session" scenario's fresh-child-and-note outcome reverse with the "Unknown task_id fallback" requirement.
- The "Portable rendering" requirement's call-label rule ("the call label SHALL be the `description` value ... and the effective `subagent_type` otherwise") reverses for `task_resume` under the resume call renderer.
- The "Tool description roster" usage note "The result names the `task_id` that a later call can reuse" rewrites to name `task_resume` as the continuation tool.
- The Security Boundaries line "Project-agent approval protects against silently consuming repository-controlled prompt files" reverses with the approval-flow removal.
- The "Subagent configuration file" requirement's key mandate (`provider`, `model`, `reasoningEffort`) rewrites to the renamed key set `provider`, `model`, `reasoning_effort`.
- The "Agent definition discovery" requirement's `reasoningEffort` frontmatter key renames to `reasoning_effort`, and its "Unknown metadata SHALL be ignored" rule gains the stale-key diagnostic exception.
- The "Tau JSON collection" requirement's provider and model recovery notes ("tell the caller to omit provider, model, and reasoning overrides") rewrite to direct the caller to the config-file or agent-definition pins, per Required Outcome 4.
- The single flat `task` tool and its camelCase field names are rewritten to the two-tool snake_case surface of Required Outcome 2.

The operator approves these reversals as part of this proposal.

Consumer evidence for the environment parameters: this repository ships no project agents and no project `.tau` directory, so no in-repo consumer's behavior changes under all-layer discovery. The current surface's own default behavior is the fixed spawn cwd this proposal requires: omission of `cwd` spawns the child in the parent session cwd. The current default `agent_scope` of `user` widens to a fixed all-layer discovery.
...
2. Every parameter is snake_case. The complete parameter surface is:
   ...
   `subagent_type` omission selects `general-purpose`; a non-empty string names an agent that call-time all-layer discovery can resolve. A name that call-time all-layer discovery cannot resolve fails closed with the roster teach-back. Discovery uses the baseline collision precedence (project replaces user, user replaces bundled). `description` is a display label with no behavioral effect. `timeout_seconds` is greater than 0, at most 10800, and defaults to 3600. `task_id` must be a non-empty string from an earlier task result, with no format check beyond non-empty after trimming. Validation trims surrounding whitespace on `subagent_type` and `task_id` as the baseline does, the trimmed value is effective, and a value empty after trimming fails closed.

3. Each tool rejects the other tool's fields and every removed field. A `task` call with `task_id` fails closed with a teach-back that names `task_resume`. A `task_resume` call with `subagent_type`, `description`, or `cwd` fails closed as an unknown field.
...
5. The dispatch environment is fixed. A fresh child spawns in the parent session's working directory; no `cwd` parameter exists, and a child that needs another directory changes its own directory or receives absolute paths in its prompt. Agent discovery is fixed to all layers: bundled agents, `~/.tau/agents` definitions, and the nearest ancestor `.tau/agents` definitions, with the collision precedence of the baseline (project replaces user, user replaces bundled). The `agent_scope` and `confirm_project_agents` parameters (today's `agentScope` and `confirmProjectAgents`) and the project-agent approval flow are removed; no per-call control narrows or widens discovery.

6. The session-agent mapping recovers the resume agent. Every fresh dispatch writes the child session id and its agent name to the session-agent mapping at `~/.tau/superpowers-subagent-sessions.json` before the child process spawns, and a failed or impossible write fails the dispatch closed, so no child ever starts without a mapping entry. Mapping writes serialize through a dedicated process-safe mapping lock held across each read-modify-write cycle, so concurrent fresh dispatches cannot lose an entry. A `task_resume` call looks up the mapped agent name, re-discovers that agent's definition by name through all-layer discovery anchored at the parent session's cwd, and regenerates the resume-variant prompt. A missing mapping entry, a missing session record, a non-subagent role, a Tau-process unknown-session diagnostic, or a mapped name that no discoverable agent provides each fails closed.

7. Resume fails closed. Under every failure condition of Required Outcome 6, the result starts zero children. In the Tau-process unknown-session condition, the attempted process's immediate CLI failure counts as no child: it performs no agent work, starts no session, and no fresh fallback child starts, and its diagnostic folds into the teach-back content. The failed result carries the teach-back as its content, no envelope, an empty `results` array, and no `planned` field. The result preserves the requested id, states that no child started, and directs the caller to `task` for a fresh child. A successful resume stays pinned to the original session and its recorded cwd.

8. Validation fails closed everywhere. Unknown fields, wrong types, empty required values, and out-of-range timeouts fail closed with teach-backs on both tools. A `task` `subagent_type` that no discoverable agent provides fails closed with the roster teach-back; on `task_resume`, `subagent_type` is an unknown field. No value is coerced, silently dropped, or reinterpreted. A removed override field, such as `reasoningEffort`, gets the unknown-field teach-back with wording that names the removal and directs the caller to the config-file or agent-definition pins. A removed environment field, such as `cwd` or `agentScope`, gets the unknown-field teach-back with wording that states the fixed behavior. The repair-note mechanism is removed entirely.

9. Each tool's schema description, tool description, and prompt guidelines describe only that tool's supported shape. `task` shows the minimal new-child example, the inheritance default, and the fixed environment, carries the all-layer roster, and names `task_resume` in a one-line continuation pointer. `task_resume` shows the continuation shape with `task_id` and `timeout_seconds`, names the session-agent mapping as the source of the resumed agent, and carries no roster, because its caller cannot select an agent: a roster on the resume tool re-creates the agent-invention surface this change removes. `task_resume` teach-backs name its three fields and the mapping, and list no agents.

10. Unchanged contracts: the result envelope and its id semantics, details schemaVersion 2 with the one-element results array, child status markers, cancellation, the timeout mechanism on both tools with the default 3600 seconds, the same-id lock applied to `task_resume` ids, the recursion guard, the dedicated background teach-back on both tools, and `tau sessions --all` visibility.
...
1. A call `{"prompt": "Find caching options"}` dispatches one `general-purpose` child that spawns in the parent session's working directory and, when no config-file or frontmatter pin resolves, inherits the parent session's provider, model, and thinking level.
...
9. A call whose `subagent_type` names a project-layer definition dispatches that definition when the nearest ancestor `.tau/agents` provides it; the roster teach-back lists the bundled, user, and project agents of the session start.
...
14. A call `{"prompt": "Continue.", "task_id": "<id>", "timeout_seconds": 7200}` to `task_resume` resumes that session with a 7200-second timeout.
...
- Fail-closed resume, fallback removal, stderr-diagnostic handling, child-exit recovery-note rewording, session-agent mapping writes, and parent-cwd spawn in `runner.py`.
- The session-agent mapping file and its read, write, and fail-closed lookup behavior.
- All-layer discovery for the tools, the snake_case rename of the config-file reasoning-effort key in `config.py`, and the same rename plus the stale-frontmatter-key diagnostic in `discovery.py`.
...
- The details schema version, usage tracking, the sidebar, and `render_task_result`; the rendering changes in scope are the `task_resume` `render_call` and the note-rendering removal in `dispatch.py`.
- Nested dispatch in any form. The recursion guard stays binary.
- Living-spec synchronization. It happens in the finishing stage of the finishing-a-development-branch skill. Its basis is this whole proposal — every Required Outcome, Scope line, and Constraint — and the Baseline Evidence discrepancy list is its minimum set of named reversals.
...
- No per-call cwd, agent-layer, or approval control exists. Discovery is fixed to all layers, and fresh children spawn in the parent session cwd.
...
One execution core, two registrations. `setup()` registers `task` and `task_resume`, each with its own schema, description, and prompt guidelines. Both execute through the `TaskDispatcher`, whose `ParsedRequest` carries an explicit mode. Validation is per tool: the `background` check precedes the `task_id` check, so a call carrying both gets the background teach-back; `task` rejects `task_id` before other argument validation, and `task_resume` requires it. Unknown fields fail closed per tool, which rejects each tool's absent fields automatically.
...
Descriptions: `task` leads with the minimal example, the inheritance rule, and the fixed environment, and carries the all-layer roster; `task_resume` leads with the continuation shape and the mapping as the agent source, with no roster. The shared prompt-guideline text keeps the dispatch threshold, the prohibition on delegating simple reads, searches, commands, or small edits, the prohibition on dispatching work the caller is about to perform itself, the self-contained-prompt requirement, and the single-task-per-call rule, split so each tool's guidelines reference only that tool. `task_resume` registers its own call renderer: its label names the resume operation and the requested id, never an agent name, because the mapped agent is not a call argument.
...
- Keeping `agent_scope` and `confirm_project_agents`. Rejected by operator decision: the operator runs the tools inside a secure sandbox, and the fixed all-layer discovery makes subagent behavior predictable while removing the approval machinery and its prompt-surface cost.
...
- Code: `extension.py` (two registrations, schemas, descriptions, guidelines, no confirmation-UI wiring), `dispatch.py` (mode split, parameter surface, teach-backs, notice and approval-flow removal), `runner.py` (fail-closed resume, fallback removal, child-exit recovery-note rewording, mapping writes, parent-cwd spawn), a new session-agent mapping module, `config.py` and `discovery.py` (key rename and stale-key diagnostics, all-layer tool discovery), `utils.py` (override-layer and cwd-override removal), `models.py` (repair-note field removal), `rendering.py` (resume call-label renderer). `catalog.py` (fail-fast teach-back wording only), `locking.py`, `usage.py`, and `sidebar.py` need no other changes.
...
- Docs: `README.md` task sections, the selection-precedence list, the custom-agents sections (both layers, no per-call scope), the isolation notes, and the operator notes including the cleanup procedure; the README announcements of removed behaviors include the different-agent resume removal; `skills/using-superpowers/references/tau-tools.md`.
...
**Project-agent exposure:** project-layer definitions are repository-controlled prompt input. This change makes them discoverable and dispatchable without a per-call approval, and their names and descriptions appear in the roster. Detection is the roster naming an unexpected project agent; recovery is removing the definition from `.tau/agents`. The operator runs the tools inside a secure sandbox, accepts repository-controlled agent prompts as dispatch input, and accepts the removal of the approval flow.
...
**Resumed-run timeout:** a `task_resume` call carries `timeout_seconds` and defaults to 3600 seconds. The same-id lock is non-blocking: the losing same-id resume fails closed immediately and never waits on the winner's timeout. A timed-out resume leaves the session's recorded turns intact, and a later resume continues the retained context.
...
**Resume availability:** resume depends on the Tau session store keeping child sessions and on the session-agent mapping keeping entries. A cleaned-up store or a deleted mapping turns a resume into a fail-closed result, never a fresh child. Resume re-discovery anchors at the parent session's cwd, matching the fresh-dispatch discovery rule, so a cross-project resume whose mapped agent is a project-layer definition fails closed; the caller starts a fresh `task` call in that project instead. The rejected alternative — anchoring re-discovery at the session's recorded cwd — finds the original project's definitions and the resume succeeds; the extension keeps one discovery rule for both tools, so re-discovery stays anchored at the parent session's cwd. Usage in details still covers the resumed run only, so totals across resumed children undercount the earlier turns. This undercount stays documented behavior.

**Resume definition drift:** resume re-discovers the mapped agent's definition by name at resume time, so a definition added later in a higher-precedence layer silently changes the resumed run's prompt and profile: a project `.tau/agents/<name>.md` shadows the user-layer original, and a new `~/.tau/agents/<name>.md` shadows a bundled agent. The operator owns the name's resolution, not a frozen definition. Detection is a resumed run whose behavior or tool policy differs from the session's origin. Recovery is removing the shadowing definition or starting a fresh `task` call with the wanted agent. The operator accepts name-based ownership.
...
**Rollback:** the trigger is a failed acceptance check or a regression the implementation stage's implement-test-fix loop cannot resolve. The steps are a revert of the branch commits, a re-install of the prior extension and skills, and a session restart. The rollback steps touch no session state: both the prior and the new surface treat pinned child sessions as ordinary store sessions, so they need no cleanup. The mapping file survives rollback as inert extension state the prior surface ignores; deleting it is safe, and on re-apply its stale entries are harmless.

**Observability:** the envelope and `details.results[].taskId` record every child session id, and `tau sessions --all` lists every child session with its model and cwd. The session-agent mapping is a plain JSON file an operator can inspect. The implementation records a post-implementation transcript in the same immutable format as the baseline transcript.

**Security:** the per-call consent boundary is removed: bundled, user, and project definitions are all dispatchable without approval, and repository-controlled agent prompts are task-surface input. The operator runs the tools inside a secure sandbox and accepts that a child's prompt remains task-controlled input, a property unchanged from the current surface.
