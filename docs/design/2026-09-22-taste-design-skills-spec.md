# Spec: Taste Design Skills

Derived from the approved proposal `docs/design/2026-09-22-taste-design-skills-proposal.md` (operator-approved version with content digest `bd8a38954ca3b0c9a94ed2a2b56bf787e444dc28`). Selected depth: Standard. Baseline branch: undocumented existing domain. No living spec covers the skill-catalog domain, so every requirement below is ADDED to the absent living spec; each requirement states complete post-change behavior, covering established unchanged behavior and requested changes together.
