...
6. A call whose `task_id` matches no session starts a fresh child with a new session id, and its result carries a repair note stating that the id matched no session. A `task_id` that matches a session whose recorded role is not the subagent role does the same, and its repair note states that the session is not a task child. A failed child's `task_error` tag wraps its final assistant message when one exists. When no final message exists, the tag wraps the OpenCode form `Subagent failed (task_id: <id>): <error>`. The envelope `id` attribute names the child session id in every case.
