...
- Tau capability evidence, recorded in the immutable transcript `docs/design/evidence/opencode-task-interface/tau-capability/transcript.txt`. The transcript was captured against the installed Tau CLI. It records each command with its complete output and exit code:
  ...
  - A resumed run honors changed settings: the transcript's appended section resumes a session created with `z-ai/glm-5.3-flash` under `z-ai/glm-5.3`, and the resumed stream records the new model.
  - `tau --session <missing-id>` exits with code 2 and prints `Unknown session: <id>` on stderr. This is a detectable clean failure.
  - The transcript does not separately exercise a changed `--cwd`, the regenerated system-prompt append, or the profile and thinking policy extensions on a resumed run. Those behaviors reuse the fresh run's argv and prompt construction, per the Resume paragraph in Approach, and the implementation adds tests that exercise them on a resumed run.
...
3. The tool description carries the agent roster with tool-policy annotations, the default selection rule, a when-not-to-use section, and usage notes in the OpenCode layout. The usage notes state these rules:
   ...
   The annotation values come from the bundled agent profiles. `general-purpose` and `implementation` use the `general-purpose` profile and render `Tools: all`. `read-only` uses the `read-only` profile and renders `Tools: read`. `code-review` and `document-review` use the `review` profile and render `Tools: read, bash`, where the review instructions govern `bash` use. The retained guidance rules, stated in full: the dispatch threshold is to delegate only work that has at least three distinct actions, is substantive, is risk-bearing, or takes more than a few tool calls. The prohibitions are to never dispatch a task and then do the same work, and never delegate an operation on the simple-operations list. The prompt requirements are a self-contained prompt and a stated verification method for the result.
