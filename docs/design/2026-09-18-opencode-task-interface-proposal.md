...
- A teach-back is a rejection or repair tool result whose content names the valid values and the exact next step.
- Fail closed means the tool rejects the call and starts no child.
...
- An eligible agent is any agent the per-call dispatch can run: bundled and user agents, plus project agents when the approval flow of Required Outcome 10 or an explicit `confirmProjectAgents: false` admits them.
- The roster is the model-visible agent list: bundled and user agents only. Teach-backs list the roster, never the eligibility set.
- A profile is an agent-definition field with the value `general-purpose`, `read-only`, or `review`, and it selects the child's tool policy. The `general-purpose` profile registers no policy extension, so every Tau tool is available. The `read-only` profile permits the `read` tool only. The `review` profile permits the `read` tool plus instruction-governed `bash`.
- A tool-policy annotation is a roster line's `Tools: <policy>` label, rendered from the profile.
- A profile policy extension is the generated child extension that enforces that policy on the child's tool-call events. The extension adds no OS-level isolation.
- The current guidance is the existing tool description's dispatch-threshold language, its prohibition sentences, and its prompt-guideline sentences. Required Outcome 3 enumerates the retained rules.
...
- Tau capability evidence, recorded in the immutable transcript `docs/design/evidence/opencode-task-interface/tau-capability/transcript.txt`. The transcript was captured against the installed Tau CLI. It records each command with its complete output and exit code:
  ...
  - A resumed run honors changed settings: the transcript's resumed-run section proves a session created with `z-ai/glm-5.3-flash` resumed under `z-ai/glm-5.3`, and the raw stream in `tau-capability/resume-changed-model.raw.jsonl` records the changed model on every assistant event.
  - A resumed run executes in the session's recorded creation cwd: the transcript's cwd section resumes a session created in `/tmp/tau-capability-evidence` with `--cwd /tmp/other-cwd`, and the child's `bash pwd` tool result in `tau-capability/resume-cwd-pwd.raw.jsonl` reports `/tmp/tau-capability-evidence`. The session record keeps its creation cwd.
  - `tau --session <missing-id>` exits with code 2 and prints `Unknown session: <id>` on stderr. This is a detectable clean failure.
  - The transcript does not separately exercise the regenerated system-prompt append or the profile and thinking policy extensions on a resumed run. Those behaviors reuse the fresh run's argv and prompt construction, per the Resume paragraph in Approach. The implementation adds tests that exercise them on a resumed run.
...
1. The tool takes exactly one task per call. The options and their semantics:
   ...
   - `prompt`: required non-empty string. The child's task. The prompt is preserved verbatim.
   - `subagent_type`: optional non-empty string naming an eligible agent. Validation trims surrounding whitespace, and the trimmed value is the effective name. Omission selects `general-purpose`. A name that no eligible agent provides fails closed with a teach-back that lists the roster.
   - `description`: optional string. Display label only, with no behavioral effect. The call label is the description when it is non-empty after trimming, and the effective `subagent_type` otherwise.
   - `task_id`: optional. A present value must be a string and non-empty after trimming, or the call fails closed with a teach-back. No format check exists beyond that. The trimmed value is the effective `task_id` for session lookup, the lock, and repair notes. A well-formed value that matches no session falls back per Required Outcome 6.
   - `cwd`: optional string directory path. Omission resolves to the parent session cwd. A relative path resolves against the parent session cwd. Any path, absolute or relative, expands `~` and is then resolved canonically to an absolute path. On a resumed run, Required Outcome 5 governs instead.
...
2. A call with an unknown field fails closed. The teach-back names the unknown fields, the roster (bundled and user agents), and one valid flat example. `background` fails closed with a dedicated teach-back. Background dispatch is not supported in this harness. The result of a `task` call arrives when the child finishes. A fail-closed validation result carries the teach-back as its content. A headless project-approval failure carries the teach-back that names the project agents directory. An interactive project-approval denial cancels the call before any child starts. The result carries the pre-session failure contract of Required Outcome 6. That flow is out of scope otherwise. A fail-closed result carries no envelope, an empty `results` array, and no `planned` field, because no child starts.
3. The tool description carries the agent roster with tool-policy annotations, the default selection rule, a when-not-to-use section, and usage notes in the OpenCode layout. The usage notes state these rules:
   ...
   The description roster and the `subagent_type` description roster are static per session. They list the agents discovered at session start from the bundled and user layers only, anchored at the session cwd, with the collision precedence of Required Outcome 1. A discovery failure at session start falls back to the bundled agents. Teach-back rosters list the same bundled and user agents. Project agents stay out of every roster and every teach-back, including by name. Their eligibility surfaces only through the approval flow of Required Outcome 10.
...
4. Model-facing result content is one task envelope: `<task id="<taskId>" state="completed|error">` wrapping a `task_result` or `task_error` tag. `taskId` is the child's Tau session id. Envelope state is the process outcome only. `completed` means the child finished and delivered a final assistant message, whatever status marker that message carries inside its text. `error` means the child failed, was cancelled, timed out, or ended with no final assistant message. Child status markers (`DONE`, `DONE_WITH_CONCERNS`, `BLOCKED`, `NEEDS_CONTEXT`) live inside the message text and do not change the envelope state.
   ...
   The inner content is the child message verbatim with no escaping. The `id` and `state` attributes carry the semantics. Structured `details` are schemaVersion 2: a one-element `results` array plus `planned` with value 1, and each result carries `taskId`.
5. A call with `task_id` resumes that child session instead of creating one. A resume run launches a new child process against the existing session. The session retains the previous messages and tool outputs. The call's `prompt` is the new user turn. The content relays the new final assistant message.

   `task_id` requires `subagent_type` to name the agent whose prompt the resumed run uses. A call with `task_id` and no `subagent_type` fails closed with a teach-back that names both fields. The runner regenerates the agent body prompt and the profile policy extensions for the resumed run. The appended prompt uses a resume variant of the isolation sentence: the session's own prior turns are the child's earlier work on this task.

   Effective provider, model, and reasoning effort resolve exactly as in a fresh call and apply to the resumed run. `timeoutSeconds` bounds the resumed run. Usage in details covers the resumed run only. Before resuming, the runner verifies through the Tau session store that the session exists and that its recorded role is the subagent role. A session that fails either check falls back per Required Outcome 6.

   The call's `subagent_type` names the agent whose body prompt the runner regenerates. The tool records no per-session agent owner. A `task_id` resumed under a different `subagent_type` continues that session with the named agent's prompt and policies.

   **Resume cwd.** A resumed run executes in the session's recorded creation cwd. The call's `cwd` does not relocate a resumed run. A resume call that carries `cwd` gets a repair note stating that the resumed run uses the session's recorded cwd. The store record keeps its creation cwd.

   **Authorization.** Authorization for resume is possession-based. Any code running as the same OS user account can list every child session with `tau sessions --all`. That code can resume any child from any parent session, including a parent in a different project. A resumed run can disclose the child's earlier content and add new content to it. The tool adds no per-caller authorization and no access auditing. Detection of cross-project access is therefore unavailable, and the operator accepts that absence.

   The accepted prevention is the single-user deployment, and the operator accepts cross-account exposure on shared machines. The store's file permissions are the operating system's defaults, and tau establishes no permission contract. The same-ID lock coordinates task-tool calls only. A direct `tau --session <id>` resume by another process is not prevented, and the operator accepts that overlap. Recovery for unwanted sessions is the store maintenance below.

   **Recovery.** The accepted recovery is store maintenance while no tau process uses the store. The store root is `~/.tau/sessions/`, with one directory per parent project, an `index.jsonl` per project directory, and one `<session-id>.jsonl` transcript per session. Each index line records the session's `id` and its transcript `path`. The steps: find the line whose `id` equals the `task_id` in the project `index.jsonl` files under the store root. That line's `path` names the transcript file, and its containing directory holds that project's `index.jsonl`. Stop the tau processes using that store. Delete the transcript file. Delete the child's line from that `index.jsonl`. Verify with `tau sessions --all` that the child no longer lists. The manager guards index updates with a process lock and atomic replacement, so editing a quiescent store avoids the race.
6. A call whose `task_id` matches no session starts a fresh child with a new session id. Its result carries a repair note stating that the id matched no session. A `task_id` that matches a session whose recorded role is not the subagent role does the same. Its repair note states that the session is not a task child.

   A failed child's `task_error` tag wraps its final assistant message when one exists. When no final message exists, the tag wraps the OpenCode form `Subagent failed (task_id: <id>): <error>`. In that form, `<id>` is the effective child session id, so it is the fresh id after a fallback. The envelope `id` attribute names the child session id in every case.

   A child that fails before Tau creates its session, through process startup failure or cancellation before startup, carries state `error` with no `id` attribute. Its `task_error` tag wraps the startup or cancellation error text. Its details entry carries no `taskId`, and `planned` keeps the value 1.
...
10. Requested definitions that resolve to the project layer require interactive approval or explicit `confirmProjectAgents: false`. Headless execution without explicit approval fails closed with a teach-back that names the project agents directory. The approval flow keeps its baseline behavior and stays out of scope otherwise. A project definition enters execution only through that approval or the explicit flag.
...
**Pinned child sessions.** `build_tau_argv` adds `--session-id <uuid4-hex>` and `--session-role subagent` to every fresh child. The generated id is the child's `taskId`, recorded on the child result, in the details entry, and as the envelope id. The runner records that id for every fresh-child attempt, including attempts that fail after startup. If tau never persisted that session record, a later resume with that id falls back per Required Outcome 6 as a fresh-child retry, and the result carries the repair note stating that the id matched no session. The `--session-role` tag keeps child sessions out of the default `tau sessions` listing.

**Resume.** When the call carries `task_id`, the runner first checks the session store. The check covers the session's existence and its recorded subagent role. Either check's failure falls back per Required Outcome 6. The runner then passes `--session <task_id>` instead of `--session-id`. Resume reuses the fresh run's argv and prompt construction. Only the session-selection flag, the isolation-sentence variant, and the absent `--cwd` differ. The runner does not pass `--cwd` on a resumed run, because tau runs the child in the session's recorded cwd. The runner regenerates the agent body prompt with the resume-variant isolation sentence. It passes the profile policy, thinking policy, overrides, and the recursion-guard environment exactly as in a fresh run. Only the new turn's events stream, so collection, usage, and final-message extraction run unchanged.
...
**Envelope.** Result content is the single-child envelope of Required Outcome 4 for every result that starts a child. Fail-closed validation results and headless approval failures carry the teach-back content of Required Outcome 2 without an envelope. An interactive denial carries the pre-session failure contract of Required Outcome 6. `details` keep schemaVersion 2 and the one-element `results` array, so the usage tracker, sidebar, and renderer contracts hold. Each result gains the additive `taskId` field, and `planned` carries the value 1.
...
- `rendering.py` (single-child frame, call label: the description when non-empty after trimming, else the effective `subagent_type`)
...
**Concurrency:** the tool sets no cap on the number of `task` calls in one message, matching OpenCode. The operator accepts the residual risk: an extreme burst can exhaust machine processes, memory, or provider quota before children finish. Detection is the streaming per-child updates plus `tau sessions --all`. Cancellation is the parent tool call's cancellation, which terminates and kills children per Required Outcome 8. Recovery is the Rollback entry. Two concurrent calls with the same `task_id` cannot both run, per Required Outcome 7. A direct `tau --session` resume of a child session bypasses the task-tool lock, and the operator accepts that overlap.

**Storage:** every fresh call leaves a persistent child session in the Tau session store, and sessions accumulate with no retention. The operator accepts this storage risk. Detection is `tau sessions --all` and the store directory size. Cleanup is the store-maintenance removal defined in Required Outcome 5: delete the child's entry from the project's `index.jsonl` and delete the child's `.jsonl` file.
...
**Envelope ids:** a fresh child's envelope id is an opaque 32-character UUID hex value that the runner generates. A resumed run keeps the existing session's id, in whatever form tau recorded at its creation, because the id is tau's session id. Resume applies no format check beyond the Required Outcome 1 rule.
...
**Security:** project agents are repository-controlled. Their names, definitions, and descriptions stay out of every model-facing roster and teach-back. They enter execution only through the approval flow or an explicit `confirmProjectAgents: false`. A repository that controls `.tau/agents` can still inject through an approved agent's prompt at the consent moment. The operator accepts that the approval prompt is the consent boundary.
