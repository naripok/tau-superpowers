...
- A child session is the Tau session that backs one child.
...
5. A call with `task_id` resumes that child session instead of creating one. The child keeps its previous messages and tool outputs. The call's `prompt` is the new user turn. The content relays the new final assistant message. `task_id` requires `subagent_type` to name the agent whose prompt the resumed run uses. A call with `task_id` and no `subagent_type` fails closed with a teach-back that names both fields. The runner regenerates the agent body prompt and the profile policy extensions for the resumed run. The appended prompt uses a resume variant of the isolation sentence: the session's own prior turns are the child's earlier work on this task. Effective provider, model, and reasoning effort resolve exactly as in a fresh call and apply to the resumed run. `timeoutSeconds` bounds the resumed run. Usage in details covers the resumed run only. Before resuming, the runner verifies through the Tau session store that the session exists and that its recorded role is the subagent role. A session that fails either check falls back per Required Outcome 6. The call's `subagent_type` names the agent whose body prompt the runner regenerates. The tool records no per-session agent owner, so a `task_id` resumed under a different `subagent_type` continues that session with the named agent's prompt and policies.
   ...
   **Authorization.** Authorization for resume is possession-based. Any code running as the same OS user account can list every child session with `tau sessions --all`. That code can resume any child from any parent session, including a parent in a different project. A resumed run can disclose the child's earlier content and add new content to it. The tool adds no per-caller authorization and no access auditing. Detection of cross-project access is therefore unavailable, and the operator accepts that absence. The accepted prevention is the single-user deployment, and the operator accepts cross-account exposure on shared machines. The store's file permissions are the operating system's defaults, and tau establishes no permission contract.
