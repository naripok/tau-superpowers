...
**Pinned child sessions.** `build_tau_argv` adds `--session-id <uuid4-hex>` and `--session-role subagent` to every fresh child. The generated id is the child's `taskId`, recorded on the child result, in the details entry, and as the envelope id. The runner records that id for every fresh-child attempt, including attempts that fail after startup. A handle whose record never persisted falls back per Required Outcome 6. The `--session-role` tag keeps child sessions out of the default `tau sessions` listing.
