...
Terms used throughout: a teach-back is a rejection or repair tool result whose content names the valid values and the exact next step. Fail closed means the tool rejects the call and starts no child. A child is one isolated Tau subagent process. A child session is the Tau session that backs one child. An eligible agent is any agent the discovery roster returns for the resolved `agentScope`: the bundled agents plus user-directory and project-directory agents. A tool-policy annotation is a roster line's `Tools: <policy>` label, generated from the agent definition's profile tool list. A profile policy extension is the generated child extension that permits exactly the agent profile's tools inside the child process. The current guidance is the existing tool description's dispatch-threshold language, its prohibition sentences, and its prompt-guideline sentences.
