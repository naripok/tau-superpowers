...
1. The tool takes exactly one task per call. The options and their semantics:
   ...
   - `task_id`: optional non-empty string. The `taskId` that a previous task result reported. Any other value falls back per Required Outcome 6.
   - `cwd`: optional string directory path. Omission resolves to the parent session cwd. A relative path resolves against the parent session cwd. An absolute path is used as given. `~` expands.
   - `agentScope`: one of `user`, `project`, `both`. Omission selects `user`. Another value fails closed.
   - `confirmProjectAgents`: boolean. Omission selects `true`.
   - `provider` and `model`: optional literal string overrides. Validation trims surrounding whitespace and rejects a value empty after trimming. A case-insensitive `default`, `inherit`, or `auto` placeholder coerces to omitted with a repair note. Resolution order is the call value, then the agent definition, then the config-file `[agents.<name>]` and `[defaults]` sections, then the parent session configuration. The `model` must be supported by the resolved provider.
...
7. Several `task` calls in one assistant message run concurrently. Each call is validated, approved, and dispatched independently. Each result carries its own envelope. One call's failure does not stop the others. Two concurrent calls that carry the same `task_id` do not both run. Exactly one starts its child; the other fails closed with a teach-back that names the same-id conflict.
...
**Resume.** When the call carries `task_id`, the runner first checks the session store. The check covers the session's existence and its recorded subagent role. Either check's failure falls back per Required Outcome 6. The runner then passes `--session <task_id>` instead of `--session-id`. The runner regenerates the agent body prompt with the resume-variant isolation sentence. It passes the profile policy, thinking policy, overrides, `--cwd`, and the recursion-guard environment exactly as in a fresh run. Only the new turn's events stream, so collection, usage, and final-message extraction run unchanged.
...
**Concurrency:** the tool sets no cap on the number of `task` calls in one message, matching OpenCode. The bound is the model's own restraint plus Tau's tool scheduling. The per-child timeout bounds each child at 10800 seconds at most, and the kill semantics apply per call. Two concurrent calls with the same `task_id` cannot both run, per Required Outcome 7. An extreme burst can exhaust machine resources and self-limits as children finish.
...
**Observability:** the envelope and `details.results[].taskId` record every child session id. `tau sessions --all` lists every child session with its model and cwd. The full test suite passes at the baseline commit `b833ebd` (267 tests per pytest). The implementation adds tests that cover every teach-back, envelope state, resume path, the same-id rejection, and the unknown-session fallback.
