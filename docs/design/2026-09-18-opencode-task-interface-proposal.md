...
8. Per-call lifecycle: the default timeout is 3600 seconds, and a call override must be greater than 0 and at most 3600. Cancellation terminates the child process, waits no more than five seconds, and kills it if necessary. Cancellation preserves partial messages and stderr and removes every temporary prompt and policy file. Partial updates stream from the single child's accepted messages.
