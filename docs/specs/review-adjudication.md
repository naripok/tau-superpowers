...
Workflow review gates dispatch `code-review` and `document-review` subagents and receive their findings. This domain describes how the main agent adjudicates those findings before it acts, how endorsed findings reach fix dispatches, how rejections go back to the reviewer for confirmation, and how maintained Critical rejections escalate to the user. It also describes the grounding material, grounded-finding format, and rejection-confirmation sections the reviewer prompt templates carry.
